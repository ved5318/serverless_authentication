...
def login_user(event, context):
    ...
    try:
        login_response = cognito_client.admin_initiate_auth(
                 ...
                 AuthParameters={'USERNAME': username,'SECRET_HASH': get_hash_userid(username),'PASSWORD': password},
                ClientMetadata={'username': username,'password': password}
                )

    except cognito_client.exceptions.NotAuthorizedException:
        return {"Message":"Deepak:The username or password is incorrect",'Error':True}
    except cognito_client.exceptions.UserNotConfirmedException:
        return {"Message":"Deepak:User is not confirmed"}
